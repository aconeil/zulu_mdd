...
repo_name = "w2v-bert-2.0-nchlt_mdd"
...
#Combined training uses zulu MDD corpus and NCHLT train corpus
com_train = concatenate_datasets([zulumdd_train, nchlt_train])
com_train = com_train.map(common.remove_special_characters)

vocab_train = com_train.map(common.extract_all_chars, batched=True, batch_size=-1, keep_in_memory=True, remove_columns=com_train.column_names)
...
com_train = com_train.cast_column("audio", Audio(sampling_rate=16_000))

com_train = com_train.map(common.prepare_dataset, remove_columns=com_train.column_names)
