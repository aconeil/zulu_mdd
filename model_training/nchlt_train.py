...
nchlt_train = nchlt_train.map(common.remove_special_characters)

vocab_train = nchlt_train.map(common.extract_all_chars, batched=True, batch_size=-1, keep_in_memory=True, remove_columns=nchlt_train.column_names)
...
nchlt_train = nchlt_train.map(common.prepare_dataset, remove_columns=nchlt_train.column_names)
